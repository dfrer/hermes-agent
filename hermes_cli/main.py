...
def cmd_chat(args):
    ...
    os.environ.setdefault("TERMINAL_CWD", os.getcwd())
    ...
    if provider_id == "zai":
        ...
        if base_url_env:
            if override == "-":
                ...
                effective_base = str(
                    zai_info.get("base_url", pconfig.inference_base_url) or pconfig.inference_base_url
                )
                print(
                    f"  Auto-detected endpoint restored → "
                    f"{zai_info.get('endpoint_label', 'Z.AI endpoint')} ({effective_base})"
                )
                print()
            elif override:
