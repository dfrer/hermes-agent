...
def _is_oauth_token(key: str) -> bool:
    ...
    Positively identify Anthropic-issued OAuth/setup tokens by format:
    - ``sk-ant-`` prefix (but not ``sk-ant-api``)
    - ``eyJ`` prefix for JWT-style OAuth tokens
    """
    if not key:
        return False
    # Regular Console API keys use x-api-key auth.
