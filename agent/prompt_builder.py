...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "</missing_context>\n"
    "\n"
    "<codex_cli_shell_safety>\n"
    "- When invoking `codex exec` from PowerShell, do not use `cd ... && codex exec ...`.\n"
    "- Prefer `codex exec -C /absolute/path ...` directly.\n"
    "- Do not use `~/...` for WSL workspaces from Windows PowerShell; use `/home/...` with `-C` or a `\\\\wsl.localhost\\...` path.\n"
    "- If the Codex task prompt is long or contains embedded quotes/newlines, pass it via stdin using a PowerShell here-string and a trailing `-` argument instead of one giant quoted shell argument.\n"
    "- Do not pass a routed Codex prompt via `$(cat file)` shell substitution; use stdin directly (`cat file | codex exec ... -`).\n"
    "- Do not pipe routed model output through `tail`, `head`, or `Select-Object` because that can hide the true exit status needed for fallback handling.\n"
    "</codex_cli_shell_safety>\n"
    "\n"
    "<routing_discipline>\n"
    ...
    "- `ability_context`, `visual_context`, `browser_vision`, and `vision_analyze` are read-only local-lane context tools, not route archetypes or routed implementation paths.\n"
    ...
    "- Valid route table: `3A | high-risk | Codex CLI (gpt-5.4)` with entitlement-gated fallbacks `Hermes CLI (glm-5.1 via zai)` then `Hermes CLI (MiniMax-M2.7 via minimax)`; `3B | marathon | Hermes CLI (glm-5.1 via zai)`; `3B | long-context | Hermes CLI (xiaomi/mimo-v2-pro via nous)`; `3C | quick-edit | Hermes CLI (MiniMax-M2.7 via minimax)`.\n"
    "- Never pair `PATH: quick-edit` with `TIER: 3B`; quick-edit is always the `3C` MiniMax route.\n"
    "- Do not reclassify to a Codex-primary route merely because another route exhausted. If `routing_status` already shows Codex entitlement blocked/unknown, `gpt-5.4` and `gpt-5.4-mini` share that same Codex gate; report fallback-chain exhaustion instead of speculating that another Codex tier will work.\n"
    ...
    "- Route selection is canonical, but actual execution eligibility is entitlement-gated. If a locked paid route is unavailable, do not invent a new tier; let `routed_exec` apply the effective fallback chain.\n"
    "- For `3B / marathon`, `routed_exec` must try `Hermes CLI (glm-5.1 via zai)` first. Locked paid fallbacks may be skipped entirely by entitlement policy.\n"
    ...
    "\n"
    "<git_authority>\n"
    "- Do not assume permission to `git commit`, `git push`, or clean up unrelated changes.\n"
